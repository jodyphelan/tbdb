...
def github_pr_comment(added_results, removed_results):
	...
	if len(added_results)>0:
		body+= "### Mutations added\n| Drug | Gene | Mutation | # Isolates | OR | OR-pval | LR | LR-pval | confidence |\n|-|-|-|-|-|-|-|-|\n"
		for r in added_results:
			body+=("|%(drug)s|%(gene)s|%(mutation)s|%(num_samples)s|%(OR)s|%(OR_pval)s|%(RR)s|%(RR_pval)s|%(confidence)s|\n" % r)
...
def main_identify_new_mutations(args):
	...
	for drug,gene,mutation in diff_added:
		...
		print(Counter([lineages[s] for s in variants[gene][mutation] if ";" not in lineages[s]]))
